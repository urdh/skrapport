...
%%  * skrapport-size10.clo
%%  * skrapport-size11.clo
%%  * skrapport-size12.clo
\documentclass{skdoc}
\let\SI\relax
\usepackage{siunitx}
...
\DeclareFile[key=size-common,preamble=\mypreamble]
    {skrapport-size-common.sty}
\DeclareFile[key=size-10,preamble=\mypreamble]{skrapport-size10.clo}
\DeclareFile[key=size-11,preamble=\mypreamble]{skrapport-size11.clo}
\DeclareFile[key=size-12,preamble=\mypreamble]{skrapport-size12.clo}
...
\begin{MacroCode}{class}
    ...
    \file_input:n{skrapport-size1\c__skrapport_ptsize_token .clo}
}
\end{MacroCode}
    \end{macro}
...
\begin{MacroCode}{class}
            ...
            \cs_gset:Nn\contentsname{Innehåll}
            \cs_gset:Nn\refname{Referenser}
            \cs_gset:Nn\figurename{Figur}
            \cs_gset:Nn\tablename{Tabell}
            \cs_gset:Nn\appendixname{Bilaga}
            \cs_gset:Nn\abstractname{Sammanfattning}
            \cs_gset:Nn\today
...
\begin{MacroCode}{class}
            ...
            \cs_gset:Nn\contentsname{Contents}
            \cs_gset:Nn\refname{References}
            \cs_gset:Nn\figurename{Figurr}
            \cs_gset:Nn\tablename{Table}
            \cs_gset:Nn\appendixname{Appendix}
            \cs_gset:Nn\abstractname{Abstract}
            \cs_gset:Nn\today
                {\int_use:N\year--\int_use:N\month--\int_use:N\day}
\end{MacroCode}
    \end{macro*}
...
\end{MacroCode}
    ...
    definitions.
\begin{MacroCode}{class}
\RequirePackage{calc,isodate,multicol,l3ctr2e}
\end{MacroCode}
...
\cs_new:Npn\__skrapport_section_nostar:nnnnnn#1#2#3#4#5#6{
    \if_int_compare:w #2>\ctr_use:n{secnumdepth} % !!!
    ...
    \addcontentsline{toc}{#1}{
        \if_int_compare:w #2>\ctr_use:n{secnumdepth} \else: % !!!
...
\begin{MacroCode}{class}
\RenewDocumentCommand\theenumi{}  {\ctr_use_arabic:n{enumi}}
\end{MacroCode}
    \end{macro*}
...
\begin{MacroCode}{class}
\RenewDocumentCommand\theenumii{} {\ctr_use_alph:n{enumii}}
\end{MacroCode}
    \end{macro*}
...
\begin{MacroCode}{class}
\RenewDocumentCommand\theenumiii{}{\ctr_use_roman:n{enumiii}}
\end{MacroCode}
    \end{macro*}
    ...
    (\(\text{A},\text{B},\text{C},\dotsc\)).
\begin{MacroCode}{class}
\RenewDocumentCommand\theenumiv{} {\ctr_use_Alph:n{enumiv}}
\end{MacroCode}
    \end{macro*}
...
\begin{MacroCode}{class}
\DeclareDocumentCommand\labelitemi{}{\textbullet}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\labelitemii}
\begin{MacroCode}{class}
\DeclareDocumentCommand\labelitemii{}{\textopenbullet}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\labelitemiii}
\begin{MacroCode}{class}
\DeclareDocumentCommand\labelitemiii{}{\normalfont\bfseries\textendash}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\labelitemiv}
\begin{MacroCode}{class}
\DeclareDocumentCommand\labelitemiv{}{\textrightarrow}
\end{MacroCode}
    \end{macro*}
...
\NewDocumentCommand\appendix{}{
    ...
    \cs_gset:Nn\thesection{\ctr_use_Alph:n{section}}
}
\end{MacroCode}
    \end{macro}
...
\cs_new:Nn\__skrapport_emphstyle:{
    \if_dim:w\fontdimen\c_one\font>\c_zero
        \itshape\bfseries
    \else:
        \itshape
    \fi:
...
\skip_gset_eq:NN\@mpfootins\footins
...
\begin{MacroCode}{class}
\DeclareDocumentCommand\theequation{}{\ctr_use_arabic:n{equation}}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\thefigure}
\begin{MacroCode}{class}
\DeclareDocumentCommand\thefigure{}{\ctr_use_arabic:n{figure}}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\thetable}
\begin{MacroCode}{class}
\DeclareDocumentCommand\thetable{}{\ctr_use_arabic:n{table}}
\end{MacroCode}
    \end{macro*}
...
\begin{MacroCode}{class}
\cs_new:Nn  \fps@figure{tb}
\cs_new:Nn\ftype@figure{1}
\cs_new:Nn  \ext@figure{lof}
\cs_new:Nn \fnum@figure{\figurename~\thefigure}
\cs_new:Nn  \fps@table {tb}
\cs_new:Nn\ftype@table {2}
\cs_new:Nn  \ext@table {lot}
\cs_new:Nn \fnum@table {\tablename~\thetable}
\end{MacroCode}
...
\DeclareDocumentEnvironment{figure}{o}{
    \exp_args:Nnx\@float{figure} % !!!
    \IfNoValueTF{#1}{[\fps@figure]}{[#1]}
}{
    \end@float
}
\DeclareDocumentEnvironment{figure*}{o}{
    \exp_args:Nnx\@dblfloat{figure} % !!!
    \IfNoValueTF{#1}{[\fps@figure]}{[#1]}
}{
    \end@dblfloat
}
...
\DeclareDocumentEnvironment{table}{o}{
    \exp_args:Nnx\@float{table} % !!!
    \IfNoValueTF{#1}{[\fps@table]}{[#1]}
}{
    \end@float
}
\DeclareDocumentEnvironment{table*}{o}{
    \exp_args:Nnx\@dblfloat{table} % !!!
    \IfNoValueTF{#1}{[\fps@table]}{[#1]}
}{
    \end@dblfloat
}
...
\begin{MacroCode}{class}
\cs_new:Nn\__skrapport_caption_name_style:{\small\bfseries}
\end{MacroCode}
    \end{macro*}
    \begin{macro*}{\__skrapport_caption_text_style:}
\begin{MacroCode}{class}
\cs_new:Nn\__skrapport_caption_text_style:{\small\itshape}
\end{MacroCode}
    \end{macro*}
\begin{MacroCode}{class}
...
\cs_new:Npn\@makecaption#1#2{
    ...
    \if_dim:w\box_wd:N\l_tmpa_box>\hsize
        {\__skrapport_caption_name_style:#1:~}
        {\__skrapport_caption_name_style:#2}
        \par
    \else:
...
\begin{MacroCode}{class}
\cs_new:Nn\@pnumwidth{1.55em}
\cs_new:Nn\@tocrmarg{2em}
\cs_new:Nn\@dotsep{1.7}
\ctr_gset:nn{tocdepth}{5}
\end{MacroCode}
...
\DeclareDocumentCommand\l@subsubsection{mm}{
    \@dottedtocline{3}{3.3em}{3.15em}{#1}{#2}
}
\end{MacroCode}
    \end{macro*}
...
\DeclareDocumentCommand\l@paragraph{mm}{
    \@dottedtocline{4}{6.45em}{4.15em}{#1}{#2}
}
\end{MacroCode}
    \end{macro*}
...
\DeclareDocumentCommand\l@subparagraph{mm}{
    \@dottedtocline{5}{10.6em}{5.15em}{#1}{#2}
}
\end{MacroCode}
    \end{macro*}
...
\bool_if:NT\g__skrapport_color_bool{
    ...
    \AtBeginDocument{
        \cs_gset_eq:NN\__skrapport_old_abstractname:\abstractname
        \cs_set:Nn\abstractname{
            \color{skrapport@titlecolor}
            \__skrapport_old_abstractname:
        }
    }
    \apptocmd{\quote}
...
\end{MacroCode}
    \begin{macro}{\__skrapport_set_font_size:nn}[2]
        ...
        {Baseline skip?} % !!!
\begin{MacroCode}{size-common}
\cs_new:Npn\__skraport_set_font_size:nn#2#3{
    \fontsize{#1}{#2} % !!!
}
\end{MacroCode}
    \end{macro}
...
\RequirePackage{expl3}
\ProvidesExplFile{skrapport-size10.clo}
...
\cs_new:Nn\@listii{
    \dim_gset_eq:NN\leftmargin\leftmarginii
    \dim_gset:Nn\labelwidth{\leftmarginii-\labelsep}
    \skip_gset:Nn\parsep {2pt plus 1pt minus 1pt}
    \skip_gset:Nn\topsep {4pt plus 2pt minus 1pt}
    \skip_gset_eq:NN\itemsep\parsep
}
\cs_new:Nn\@listiii{
    \dim_gset_eq:NN\leftmargin\leftmarginiii
    \dim_gset:Nn\labelwidth{\leftmarginiii-\labelsep}
    \skip_gset_eq:NN\parsep\c_zero_skip
    \skip_gset:Nn\topsep   {2pt plus 1pt minus 1pt}
    \skip_gset:Nn\partopsep{1pt plus 0pt minus 1pt}
    \skip_gset_eq:NN\itemsep\topsep
}
\cs_new:Nn\@listiv{
    \dim_gset_eq:NN\leftmargin\leftmarginiv
    \dim_gset:Nn\labelwidth{\leftmarginiv-\labelsep}
}
\cs_new:Nn\@listv{
    \dim_gset_eq:NN\leftmargin\leftmarginv
    \dim_gset:Nn\labelwidth{\leftmarginv-\labelsep}
}
\cs_new:Nn\@listv{
    \dim_gset_eq:NN\leftmargin\leftmarginvi
    \dim_gset:Nn\labelwidth{\leftmarginvi-\labelsep}
}
\endinput
\end{MacroCode}
...
\RequirePackage{expl3}
\ProvidesExplFile{skrapport-size11.clo}
...
\cs_new:Nn\@listii{
    ...
    \dim_gset:Nn\labelwidth{\leftmarginii-\labelsep}
    \skip_gset:Nn\parsep {  2pt plus 1pt minus 1pt}
    \skip_gset:Nn\topsep {4.5pt plus 2pt minus 1pt}
    \skip_gset_eq:NN\itemsep\parsep
}
\cs_new:Nn\@listiii{
    \dim_gset_eq:NN\leftmargin\leftmarginiii
...
\RequirePackage{expl3}
\ProvidesExplFile{skrapport-size12.clo}
...
\cs_new:Nn\@listii{
    \dim_gset_eq:NN\leftmargin\leftmarginii
    \dim_gset:Nn\labelwidth{\leftmarginii-\labelsep}
    \skip_gset:Nn\parsep {2.5pt plus 1pt   minus 1pt}
    \skip_gset:Nn\topsep {  5pt plus 2.5pt minus 1pt}
    \skip_gset_eq:NN\itemsep\parsep
}
\cs_new:Nn\@listiii{
    \dim_gset_eq:NN\leftmargin\leftmarginiii
    \dim_gset:Nn\labelwidth{\leftmarginiii-\labelsep}
    \skip_gset_eq:NN\parsep\c_zero_skip
    \skip_gset:Nn\topsep   {2.5pt plus 1pt minus 1pt}
    \skip_gset:Nn\partopsep{  1pt plus 0pt minus 1pt}
    \skip_gset_eq:NN\itemsep\topsep
}
\cs_new:Nn\@listiv{
