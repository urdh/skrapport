...
\begin{filecontents}{chslacite.bib}
    @article{kpfonts,
...
\begin{document}
    ...
    \changes{0.04}{Added \textsf{microtype} package}
    ...
    \changes{0.07}{Various bugfixes, \hologo{XeLaTeX} compatibility, 
                    ...
                    \texttt{intlimits} option to \textsf{amsmath}}
    \changes{0.07a}{Fixed \textsf{kpfonts} issues}
    \changes{0.09}{Introduced \textsf{kvoptions},
                    fixed abstract in twocolumn mode}
    \changes{0.10}{Fixed documentation, include skmath if exists
                    and wanted.}

    % Metadata
    \package[ctan,vcs=https://github.com/urdh/skrapport]{skrapport}
    ...
    implemented using \pkg{multicol}. The default is \texttt{onecolumn}.
    ...
    are always loaded. The default is \texttt{swe}.
    ...
    default value is \texttt{nocolor}.
    ...
    \texttt{nomath} option will supress this behaviour.

    \subsection{Further documentation (unfinished)}
    In general, the class defines the same macros as the \textsf{article} class.

    \DescribeMacro\regarding{<text>}
    The class does add a \cs{regarding} macro, which is used like the standard
        \cs{author} and \cs{title} macros and should be given an accurate but short
    ...
    In \texttt{twocolumn} mode, the package defines an environment \texttt{onecol}
      which typesets its contents in a single column. Additionally, it redefines
      \texttt{figure} and \texttt{table} as non-floats, leaving the starred versions
      intact.
    \DescribeMacro\colortheme{<theme>}
    If the package is loaded with the \texttt{color} option, changing the color theme is
    possible using \texttt{colortheme}, which loads an
    appropriate package. At the moment, four color themes are available: \textsf{default},
    \textsf{unscathed}, \textsf{cruelwater} and \textsf{violet}.
...
\end{MacroCode}
    ...
    Load \texttt{leqno.clo} and \texttt{fleqno.clo} if those options are desired.
\begin{MacroCode}{class}
