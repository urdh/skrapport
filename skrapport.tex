...
\begin{document}
    ...
    \Option{titles}\WithValues{rm, it, bf, sf, sc}\AndDefault{bf}
    ...
    the italic roman font, the boldfaced roman font or the sans serif 
    font. For historical reasons the default is a boldfaced roman font,
    but the sans serif option is very handsome.

    The final option value, \opt{titles=sc}, has additional effects.
    In addition to setting the title in small caps, the option makes
    titles centered and applies some other styling.
...
\ProvidesExplClass{skrapport}%
    {2013/09/19}{0.12d}{stylish report document class}
\end{MacroCode}
...
\end{MacroCode}
    \end{macro}
    \begin{macro}{\g__skrapport_sctitles_bool}
    Are we using the special formatting of small-caps titles?
\begin{MacroCode}{class}
\bool_new:N\g__skrapport_sctitles_bool
\bool_gset_false:N\g__skrapport_sctitles_bool
\end{MacroCode}
    \end{macro}
...
\end{MacroCode}
    \begin{macro*}{\__skrapport_title_style:}
        ...
        \changes{0.12e}{Added \texttt{sc} option}
\begin{MacroCode}{class}
        ...
        {sc}{
            \cs_new_protected:Nn\__skrapport_title_style:{\rmfamily}
            \bool_gset_true:N\g__skrapport_sctitles_bool
        }
\end{MacroCode}
    \end{macro*}
...
\end{MacroCode}
    ...
    \begin{option}{titles}{rm, it, bf, sf, sc}
...
\cs_new_protected:Nn\__skrapport_abstract_style:{
    \__skrapport_title_style:
    \bool_if:NT\g__skrapport_sctitles_bool{\scshape}
}
\end{MacroCode}
    \end{macro*}
\begin{MacroCode}{class}
...
\cs_set_protected:Nn\__skrapport_section_style:{
    \normalfont
    \bool_if:NTF\g__skrapport_sctitles_bool
        {\__skrapport_title_style:\scshape}
        {\LARGE\__skrapport_title_style:}
}
\end{MacroCode}
    \end{macro*}
