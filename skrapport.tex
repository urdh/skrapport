...
\end{MacroCode}
    We load \pkg{inconsolata} as our monospace font.
...
\begin{MacroCode}{class}
    \dim_set_to_wd:Nn\l_tmpa_dim{\bfseries\abstractname}
    \dim_set:Nn\l_tmpb_dim
        {\textwidth-\l_tmpa_dim-\c__skrapport_abstract_separator_dim}
    \begin{minipage}[t]{\l_tmpa_dim}
        \begin{flushright}
            \leavevmode\bfseries\__skrapport_abstract_style:
...
\cs_new:Npn\__skrapport_post_section:n#1{
    \skip_set:Nn\l_tmpa_skip{#1}
    \int_compare:nTF{\l_tmpa_skip>\c_zero}{
        \par\nobreak
        \skip_vertical:N\l_tmpa_skip
        \@afterheading % !!!
    }{
        ...
        \everypar{
            \if@noskipsec
                ...
                {\box_set_to_last:N\c_zero}
                \clubpenalty\@M % !!!
                \group_begin:
                    \cs_if_exist_use:N\__skrapport_saved_section:
                \group_end:
                \unskip
                \skip_set:Nn\l_tmpa_skip{-#1}
...
\end{MacroCode}
    ...
    There is a bug in this macro; the call to \cs{addcontentsline}
    doesn't expand the argument to \cs{numberline} as it should. This
    means that all section numbers are 0 in the TOC since the TOC file
    contains \emph{e.g.} \cs{thesubsection} instead of \enquote{1.1}.
\begin{MacroCode}{class}
...
\DeclareDocumentEnvironment{figure}{o}{
    \exp_args:Nno\@float{figure} % !!!
    {\IfNoValueTF{#1}{[\fps@figure]}{[#1]}}
}{
    \end@float
}
\DeclareDocumentEnvironment{figure*}{o}{
    \exp_args:Nno\@dblfloat{figure} % !!!
    {\IfNoValueTF{#1}{[\fps@figure]}{[#1]}}
}{
    \end@dblfloat
}
...
\DeclareDocumentEnvironment{table}{o}{
    \exp_args:Nno\@float{table} % !!!
    {\IfNoValueTF{#1}{[\fps@table]}{[#1]}}
}{
    \end@float
}
\DeclareDocumentEnvironment{table*}{o}{
    \exp_args:Nno\@dblfloat{table} % !!!
    {\IfNoValueTF{#1}{[\fps@table]}{[#1]}}
}{
    \end@dblfloat
}
