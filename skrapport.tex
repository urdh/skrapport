...
\begin{document}
    ...
    \Options{font}\WithValues{nofont, lmodern, palatino, kpfonts, minion}\AndDefault{kpfonts}
    In addition, the specific font can also be changed --- the available
    choices are \texttt{lmodern} (Latin Modern), \texttt{kpfonts}
    (Kp-Fonts), \texttt{minion} (Adobe Minion Pro)
    and \texttt{palatino} (either \TeX-Gyre Pagella or
...
\ProvidesClass{skrapport}%
    [2013/01/09 v0.12 Skånings rapportklass]
\end{MacroCode}
...
\end{MacroCode}
    ...
    \subsection{Messages}
\begin{MacroCode}{class}
\msg_new:nnn{skrapport}{old-font-deprecated}{The old font command \string#1 is deprecated! Use \string#2 instead.}
\end{MacroCode}
...
\DeclareVoidOption{minion}{\skrapport@SetupFont}
